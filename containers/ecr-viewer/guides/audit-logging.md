...
The eCR Viewer's has implemented an ONC-compliant audit log. It is only populated when using the `DUAL` or `NON_INTEGRATED` configurations of the viewer. In `INTEGRATED` mode, it is the responsibility of the integrating tool to log accesses to the eCRs.
...
- The audit event creation timestamp is synchronously generated by the database at the time of write to the database, within 5 seconds of the event occurring
...
- How do we want to do this? Do we need a page like the query connector is the DB's native functionality sufficient?
