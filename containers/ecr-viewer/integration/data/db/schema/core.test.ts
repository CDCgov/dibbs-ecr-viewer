...
*/    
    ...
    describe("ecr_data table", () => {
        ...
        describe('data_source column', () => {
            it('exists', async () => {
                expect(await utils.columnExistsByName(db, schema, "ecr_data", 'data_source')).toBe(true);
            });
            it('is a varchar(20)', async () => {
                const column = await utils.getColumn(db, schema, "ecr_data", 'data_source');
        ...
        describe("patient_name_first column", () => {
            it("exists", async () => {
                expect(await utils.columnExistsByName(db, schema, "ecr_data", "patient_name_first")).toBe(true);
            });
            it('is a varchar(100)', async () => {
                const column = await utils.getColumn(db, schema, "ecr_data", "patient_name_first");
                expect(column?.type).toBe("varchar(100)");
            });
        });
        describe("patient_name_last column", () => {
            it("exists", async () => {
                expect(await utils.columnExistsByName(db, schema, "ecr_dat", "patient_name_last")).toBe(true);
            });
            it('is a varchar(100)', async () => {
                const column = await utils.getColumn(db, schema, "ecr_data", "patient_name_last");
                expect(column?.type).toBe("varchar(100)");
            });
        });
        ...
        describe("report_date column", () => {
            it("exists", async () => {
                expect(await utils.columnExistsByName(db, schema, "ecr_data", "report_date")).toBe(true);
            });
            it("is a date", async () => {
                const column = await utils.getColumn(db, schema, "ecr_data", "report_date");
