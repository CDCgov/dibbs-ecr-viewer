      ...
      if (metadata.patient_addresses) {
        for (const address of metadata.patient_addresses) {
          ...
          await addressInsertRequest
            ...
            .query(
              "INSERT INTO dbo.ecr_addresses (eICR_ID, use, type, text, city, district, state, postal_code, country, period_start, period_end) VALUES (@eICR_ID, @use, @type, @text, @city, @district, @state, @postal_code, @country, @period_start, @period_end)",
            );

          if (address.line) {
            for (let i = 0; i < address.line.length; i++) {
              const addressLineInsertRequest = new sql.Request(transaction);
              await addressLineInsertRequest
                .input("line", sql.VarChar(255), address.line[i])
                .query(
                  "INSERT INTO dbo.ecr_address_lines (eICR_ID, line, rank) VALUES (@eICR_ID, @line)",
                );
            }
          }
        }
