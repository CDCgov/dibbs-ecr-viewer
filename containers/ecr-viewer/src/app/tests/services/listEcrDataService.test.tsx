  ...
  describe("get total ecr count", () => {
    it("should call db to get all ecrs", async () => {
      ...
      expect(database.one).toHaveBeenCalledWith(
        "SELECT count(DISTINCT ed.eICR_ID) FROM ecr-viewer.ecr_data as ed LEFT JOIN ecr-viewer.ecr_rr_conditions erc on ed.eICR_ID = erc.eICR_ID WHERE $[whereClause]",
        { whereClause: expect.any(Object) },
      );
    });
  ...
  describe("generate filter conditions statement", () => {
    it("should add conditions in the filter statement", () => {
      expect(
        ...
        "ed.eICR_ID IN (SELECT DISTINCT ed_sub.eICR_ID FROM ecr-viewer.ecr_data ed_sub LEFT JOIN ecr-viewer.ecr_rr_conditions erc_sub ON ed_sub.eICR_ID = erc_sub.eICR_ID WHERE erc_sub.condition IS NOT NULL AND (erc_sub.condition ILIKE '%Anthrax (disorder)%'))",
      );
    });
    it("should only look for eCRs with no conditions when de-selecting all conditions on filter", () => {
      expect(generateFilterConditionsStatement([""]).toPostgres()).toEqual(
        "ed.eICR_ID NOT IN (SELECT DISTINCT erc_sub.eICR_ID FROM ecr-viewer.ecr_rr_conditions erc_sub WHERE erc_sub.condition IS NOT NULL)",
      );
    });
  ...
  describe("generate where statement", () => {
    it("should generate where statement using search and filter statements", () => {
      expect(
        ...
        "(ed.patient_name_first ILIKE '%blah%' OR ed.patient_name_last ILIKE '%blah%') AND (ed.date_created >= '2024-12-01T00:00:00.000-05:00' AND ed.date_created <= '2024-12-02T00:00:00.000-05:00') AND (ed.eICR_ID IN (SELECT DISTINCT ed_sub.eICR_ID FROM ecr-viewer.ecr_data ed_sub LEFT JOIN ecr-viewer.ecr_rr_conditions erc_sub ON ed_sub.eICR_ID = erc_sub.eICR_ID WHERE erc_sub.condition IS NOT NULL AND (erc_sub.condition ILIKE '%Anthrax (disorder)%')))",
      );
    });
    ...
    it("should generate where statement using filter conditions statement (no search provided)", () => {
      expect(
        ...
        "(NULL IS NULL OR NULL IS NULL) AND (ed.date_created >= '2024-12-01T00:00:00.000-05:00' AND ed.date_created <= '2024-12-02T00:00:00.000-05:00') AND (ed.eICR_ID IN (SELECT DISTINCT ed_sub.eICR_ID FROM ecr-viewer.ecr_data ed_sub LEFT JOIN ecr-viewer.ecr_rr_conditions erc_sub ON ed_sub.eICR_ID = erc_sub.eICR_ID WHERE erc_sub.condition IS NOT NULL AND (erc_sub.condition ILIKE '%Anthrax (disorder)%')))",
      );
    });
  });
});
