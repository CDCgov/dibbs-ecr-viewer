...
def test_build_fhir_converter_request_forwards_fhir_xml_unchanged():
    fhir_xml_bytes = (
        b'<?xml version="1.0" encoding="UTF-8"?>\r\n'
        b'<Bundle xmlns="http://hl7.org/fhir">\r\n'
        b'  <type value="document"/>\r\n'
        b"</Bundle>\r\n"
    )
    fhir_xml = fhir_xml_bytes.decode("utf-8")
    orchestration_request = {
        "message_type": "ecr",
        "data_type": "ecr",
        "config_file_name": "bundle-only.json",
    ...
    result = build_fhir_converter_request(fhir_xml, orchestration_request)

    assert orchestration_request["message_type"] == "ecr"
    assert result["input_type"] == "ecr"
    assert result["root_template"] == "EICR"
    assert result["input_data"].encode("utf-8") == fhir_xml_bytes
