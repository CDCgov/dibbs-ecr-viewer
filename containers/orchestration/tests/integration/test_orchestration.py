...
def test_health_check(setup):
    ...
    for port_number in port_number_strings:
        ...
        assert service_response.status_code == 200, (f"Expected status code 200, but got ${service_response.status_code}. Response content is ${service_response.content}")


@pytest.mark.integration
def test_openapi():
    actual_response = httpx.get(ORCHESTRATION_URL + "/orchestration/openapi.json")
    assert actual_response.status_code == 200, (f"Expected status code 200, but got ${actual_response.status_code}. Response content is ${actual_response.content}")
...
def test_process_message_endpoint(setup, clean_up_db):
    ...
    assert orchestration_response.status_code == 200, (f"Expected status code 200, but got ${orchestration_response.status_code}. Response content is ${orchestration_response.content}")
...
def test_failed_save_to_ecr_viewer(setup, clean_up_db):
    ...
    with open(
        ...
        assert orchestration_response.status_code == 500, (f"Expected status code 500, but got ${orchestration_response.status_code}. Response content is ${orchestration_response.content}")
